...
class RedisMemorySessionService(BaseSessionService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.cache = (
            redis.Redis.from_url(uri, **kwargs)
            if uri
            else redis.Redis(host=host, port=port, db=db, **kwargs)
        )
    ...
    async def create_session(
        ...
        *,
        ...
        user_sessions_key = f"{State.APP_PREFIX}:{app_name}:{user_id}"
    ...
    async def append_event(self, session: Session, event: Event) -> Event:
        ...
        user_sessions_key = f"{State.APP_PREFIX}:{session.app_name}:{session.user_id}"
        pipe.expire(user_sessions_key, self.expire)
    ...
    async def _load_sessions(self, app_name: str, user_id: str) -> dict[str, dict]:
        ...
        try:
            ...
            raw_sessions = await self.cache.mget(session_keys)
            ...
            for i, raw_session in enumerate(raw_sessions):
                ...
                if raw_session:
                    try:
                        sessions[session_id] = json.loads(raw_session)
                    except (json.JSONDecodeError, UnicodeDecodeError) as e:
                        logger.error(f"Error decoding session {session_id}: {e}")
                else:
                    logger.warning(
                        "Session ID %s found in user set but session data is"
                        " missing. Cleaning up.",
                        session_id,
                    )
